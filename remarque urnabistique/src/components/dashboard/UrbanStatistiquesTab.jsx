...
export default function UrbanStatistiquesTab({ onSwitchTab }) {
  ...
  const [period, setPeriod] = useState('week'); // week or month (simulated as Sem 1-4 for now)
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const zonesData = await getZonesWithStats();
      ...
      if (isZoneSelected) {
        ...
        // Aggregate global stats manually from remarks
        const remarks = await getValidatedRemarks(null);
